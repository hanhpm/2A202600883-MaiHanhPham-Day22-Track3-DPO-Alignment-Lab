...
# Hyperparameters from deck Â§5.2 (TRL DPOTrainer implementation frame)
...
pref_ds = Dataset.from_pandas(pref_df, preserve_index=False)
...
metrics = {
    ...
    "end_chosen_reward": float(last_chosen) if "last_chosen" in globals() and chosen_col else None,
    "end_rejected_reward": float(last_rejected) if "last_rejected" in globals() and rejected_col else None,
    "end_reward_gap": float(last_gap) if "last_gap" in globals() and chosen_col and rejected_col else None,
}
